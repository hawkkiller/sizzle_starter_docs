...
In Sizzle Starter, there is an abstract class named `Dependencies` and two implementations:

- `DependenciesMutable` - used while initializing the dependencies.
This is because the dependencies may need to be changed during initialization,
such as when they are being configured.

- `DependenciesImmutable` - used in the application after the initialization.
This is because the dependencies should be immutable after initialization,
...
returning the `DependenciesImmutable` instance.
...
(basically instance of `DependenciesImmutable`). At this moment,
